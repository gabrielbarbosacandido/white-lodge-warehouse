...
| `task clean` | Remove the warehouse and dbt artifacts, keeping unpacked source files |
| `task reset` | Return `data/` to its source state, keeping only `sample-data.tar.gz` |
| `task nuke` | `clean` plus containers, volumes and the NADAC cache |
...
└── data/
    ├── README.md                 committed
    ├── sample-data.tar.gz        committed. The only input
    ├── landing/                  generated by `task data`, not committed
    ├── external/nadac/           generated by `task ingest`, not committed
    └── warehouse/                generated by `task transform`, not committed
```
